...
class WidgetFactory(object):
    def __init__(self, name, app_root=None, quiet=False):
        ...
        self.widget_dir = os.path.join(app_root, "static", "widgets", self.name)
        self.job_file = os.path.join(app_root, "jobs", "%s.py" % (self.name,))
        template_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "templates", "widget")
        )
        self.env = Environment(
            loader=FileSystemLoader(template_path), keep_trailing_newline=True
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-l", "--list", dest="list_widgets", action="store_true", help="List widgets"
    )
    parser.add_argument(
        "-r", "--remove", dest="remove", action="store_true", help="Remove widget"
    )
